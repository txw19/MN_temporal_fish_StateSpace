...
cat("
  model {
    ...
    # Random intercept
     for (i in 1:S){
        delta[i] ~ dnorm(0,tau.delta)T(-1,1)
     } 

    for (t in 1:(T-1)){       #loop over year
      for (i in 1:S){         #loop over site
        r.proc[t,i] ~ dnorm(0 , tau.proc)
          r[t,i] <- int.r + delta[i] + r.proc[t,i] + beta[1] * dd[t,i] + beta[2] * coefvar[t,i] + 
    ...
    sigma.delta ~ dunif(0, 0.2)         # Prior for sd of random site effect for r
...
parameters <- c("int.r", "sigma.obs", "sigma.proc", "sigma.delta", "beta")
